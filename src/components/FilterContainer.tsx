...
const FilterContainer: FC<FilterContainerProps> = ({
  ...
  const handleTagClick = (item: string) => {
    if (selectedFilterKeyword.includes(item)) {
      const updatedKeywords = selectedFilterKeyword.filter((keyword) => keyword !== item);
    ...
    <PageContainer style={{ padding: '0', width: '100%' }}>
      ...
      {filterPageNum === 0 &&
        <>
          <FilterBarContainer>
            ...
            />
            <div style={{
              ...fontStyles.heading3Semibold, color: `${colors.grey100}`, flex: '8',
          ...
          <MusicianFilterContainer>
            <div style={{
              ...fontStyles.heading2Medium, color: `${colors.textActive}`, height: '27px',
              lineHeight: '27px', marginBottom: '4px'
            }}>찾고 싶은 뮤지션 종류를 선택해주세요.</div>
            <div style={{
              ...fontStyles.subheadingRegular, color: `${colors.textDefault}`, height: '21px',
              lineHeight: '21px', marginBottom: '24px'
            }}>다중 선택 할 수 있어요.</div>
            <MusicianFilterWrapper>
              {filterKeyword.map((item) => {
                return (
                  <MusicianFilterTag
                    style={{
                      backgroundColor: selectedFilterKeyword.includes(item) ? `${colors.primary100}` : '',
                      color: selectedFilterKeyword.includes(item) ? `${colors.textDarken}` : '',
                      border: selectedFilterKeyword.includes(item) ? `1px solid ${colors.primary100}` : '',
                    }}
                    onClick={() => handleTagClick(item)}
                  >{item}</MusicianFilterTag>
                )
              })}
            </MusicianFilterWrapper>
            <Button
        ...
        <>
          ...
          <MusicianFilterContainer>
            <div style={{
              ...fontStyles.heading2Medium, color: `${colors.textActive}`, height: '27px',
              lineHeight: '27px', marginBottom: '4px'
            }}>찾고 싶은 뮤지션의 수준을 선택해주세요.</div>
            <div style={{
              ...fontStyles.subheadingRegular, color: `${colors.textDefault}`, height: '21px',
              lineHeight: '21px', marginBottom: '28px'
            }}>하나만 선택 할 수 있어요.</div>
      ...
      {filterPageNum === 2 &&
        <>
          ...
          <div style={{
            padding: '0 16px', marginTop: '20px', display: 'flex',
