...
export default function PostCreator() {
  ...
  const [isCategorySelecting, setIsCategorySelecting] = useState<boolean>(false);
  ...
  const handleImageDelete = (image: Attachment) => {
    const updatedAttachments = attachments.filter((item: Attachment) => item !== image);
    setAttachments(updatedAttachments);
  };
      ...
      {showRulePage
        ...
        <>
          <StyledNavigation>
            <img src={backIcon} alt='뒤로 가기'
              onClick={() => navigate(-1)}
              style={{ marginLeft: '9px', cursor: 'pointer' }} width='24px'
            ></img>
            <h1 style={{
          ...
          <div style={{
            width: '100%', maxWidth: '500px', padding: '0 16px 0 16px', marginTop: '20px',
            marginLeft: 'auto', marginRight: 'auto', boxSizing: 'border-box'
          }}>
            <div style={{
              ...fontStyles.subheadingMedium, color: `${colors.textActive}`, height: '24px'
            }}>
              이용약관</div>
            <div style={{
              ...fontStyles.caption2Medium, color: `${colors.textDefault}`
            }}>어쩌구 저쩌구
            </div>
          </div>
        </>
        ...
        <PageContainer style={{ padding: '0', width: '100%' }}>
          ...
          <TopBar>
            ...
            <div style={{
              display: 'flex', flex: '8', justifyContent: 'center', alignItems: 'center'
            }}>게시글 작성</div>
          ...
          <div style={{ padding: '0 16px' }}>
            ...
            <div style={{
              ...
              height: '21px', marginBottom: '5px', lineHeight: '21px'
            }}>
              게시판 이용규칙을 준수하며 작성해주세요.
            </div>
            <div
              style={{
                ...fontStyles.subheadingRegular, display: 'flex', height: '24px',
                color: `${colors.textDefault}`, position: 'relative', marginBottom: '30px',
              }}
            >
              <div
                onClick={() => { setShowRulePage(true); setPreventPopstate(!preventPopstate); }}
                style={{ lineHeight: '24px', cursor: 'pointer' }}
              >게시판 이용규칙</div>
              <img
                onClick={() => { setShowRulePage(true); setPreventPopstate(!preventPopstate); }}
                width='24px' height='24px' style={{ cursor: 'pointer' }}
                src={rightArrowButton} alt="게시판 이용규칙 이동"
              ></img>
            </div>
            ...
            <div style={{ position: 'relative' }}>
              <PostCategoryWrapper
                style={{ border: isCategorySelecting ? `1px solid ${colors.primary100}` : '' }}
                onClick={() => setIsCategorySelecting(!isCategorySelecting)}
              >
                <div style={{ color: selectedCategory ? `${colors.textActive}` : `${colors.textMuted}` }}>
                  {selectedCategory ? selectedCategory : '게시글의 카테고리를 선택하세요.'}
                </div>
                <div>
              ...
              <div style={{
                display: isCategorySelecting ? 'flex' : 'none', position: 'absolute', top: '48px', left: '0px',
                border: `1px solid ${colors.primary100}`, borderRadius: '8px', width: '100%', height: '128px',
                backgroundColor: `${colors.bgInput}`, padding: '20px 16px 12px 16px', boxSizing: 'border-box',
                flexDirection: 'column', gap: '12px', alignItems: 'flex-start'
              }}>
                <CategoryList onClick={() => {
                  ...
                  }} src={checkIcon} alt="Checked Category" />
                </CategoryList>
                <CategoryList onClick={() => {
            ...
            />
            <div style={{
              ...fontStyles.body3Regular, color: `${colors.textDefault}`, textAlign: 'end', height: '19px'
            }}>
              {postContent.length}/200
            </div>
            <hr style={{ margin: '15px 0 20px 0', border: 'none', borderTop: `1px solid ${colors.grey05}` }}></hr>
          </div>

          {/* 사진 */}
          <div style={{
            width: 'calc(100% - 32px)', marginLeft: 'auto', marginRight: 'auto',
            position: 'relative', marginBottom: '70px'
          }}>
            <div style={{ display: 'flex', justifyContent: 'flex-start', marginTop: '4px' }}>
              <CameraStyle>
                <label htmlFor="ex_file">
                  <div style={{
                    height: '64px', width: '64px', boxSizing: 'border-box', borderRadius: '5px',
                    border: `1px solid ${colors.grey100}`, display: 'flex', justifyContent: 'center',
                    alignItems: 'center'
                  }}>
                    <div style={{ height: '39px', display: 'flex', flexDirection: 'column' }}>
                      <img src={attachImageIcon} alt="btnStart" width='24px' />
                      <div style={{
                        color: `${colors.textActive}`, ...fontStyles.caption1Regular,
                        textAlign: 'center', height: '13px'
                      }}>{attachments.length}/5</div>
                    </div>
                  </div>
                </label>
                <input
              ...
              <Swiper
                style={{ width: 'calc(100% - 74px)', height: '64px', zIndex: '0', flexShrink: '0' }}
              ...
              >
                {attachments.map((image, index) => (
                  <SwiperSlide
                    ...
                    <div key={index} style={{
                      display: 'inline-block', marginRight: index === attachments.length - 1 ? '' : '10px',
                      position: 'relative',
                    }}>
                      <div
                        style={{
                      ...
                      >
                        <img src={deleteImageIcon} alt="x" width='16px' height='16px'></img>
                      </button>
                    </div>
                  </SwiperSlide>
                ))}
...
const CategoryList = styled.div`
  width: 100%;
  height: 24px;
  color: ${colors.textActive};  font-size: 14px;
  font-weight: 500;
  cursor: pointer;
  display: flex;
  justify-content: space-between;
